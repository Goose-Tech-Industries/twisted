...
const TYPE_META = {
  // --- World ---
  tile_type:   { pk: 'id', tables: ['game_tile_types'] },
  theme:       { pk: 'id', tables: ['game_themes'] },
  capsule:     { pk: 'id', tables: ['game_capsule_items'] },
  dm_campaign: { pk: 'id', tables: ['game_dm_campaigns'] },
  dm_session_log: { pk: 'id', tables: ['game_dm_session_log'] },
  scheduled_event: { pk: 'id', tables: ['game_scheduled_events'] },
  structure_template: { pk: 'id', tables: ['game_structure_templates'] },
  ground_item: { pk: 'id', tables: ['game_map_ground_items'] },
  deployed_structure: { pk: 'id', tables: ['game_deployed_structures'] },
...
// AI content generation endpoint for AdminSauce
router.post('/admin/ai-generate', requireStaff, async (req, res) => {
  try {
    const ai = require('../server/ai-features');
    const { type } = req.body;

    if (type === 'quest') {
      const quest = await ai.generateQuest(db, {
        playerLevel: req.body.level || 5,
        mapName: req.body.location || 'Unknown',
      });
      return res.json({ success: !!quest, data: quest, message: quest ? undefined : 'Generation failed — check AI config.' });
    }

    if (type === 'item_text') {
      const text = await ai.generateItemText(db, { itemName: req.body.itemName, itemType: req.body.itemType });
      return res.json({ success: !!text, text, message: text ? undefined : 'Generation failed.' });
    }

    if (type === 'lore') {
      const text = await ai.generateLore(db, { topic: req.body.topic });
      return res.json({ success: !!text, text, message: text ? undefined : 'Generation failed.' });
    }

    if (type === 'rumor') {
      const text = await ai.generateRumor(db, { npcName: req.body.npcName, mapName: req.body.mapName });
      return res.json({ success: !!text, text, message: text ? undefined : 'Generation failed.' });
    }

    if (type === 'dm_assist') {
      const text = await ai.dmAssist(db, { dmInstruction: req.body.instruction });
      return res.json({ success: !!text, text, message: text ? undefined : 'Generation failed.' });
    }
    ...
    if (type === 'biography') {
      ...
      const [[char]] = await db.query('SELECT * FROM characters WHERE id=?', [charId]).catch(() => [[null]]);
      ...
      const text = await ai.generateBiography(db, {
        playerName: char.name, race: race?.name, className: cls?.name,
        level: char.level, questsCompleted: 0, kills: 0, deaths: 0,
      });
      return res.json({ success: !!text, text, message: text ? undefined : 'Generation failed.' });
    }

    res.json({ success: false, message: `Unknown generation type: ${type}` });
  } catch (e) { res.json({ success: false, message: e.message }); }
});

// Themes — public (players need to load themes too)
router.get('/admin/themes', async (req, res) => {
  try {
    const [themes] = await db.query('SELECT * FROM game_themes WHERE is_active=1 ORDER BY sort_order');
    res.json({ success: true, themes });
  } catch (e) { res.json({ success: true, themes: [] }); }
});

router.get('/admin/tile-palette', requireStaff, async (req, res) => {
  try {
    const [rows] = await db.query('SELECT id, name, color, category, sort_order, is_passable FROM game_tile_types WHERE is_active=1 ORDER BY sort_order');
    res.json({ success: true, tiles: rows });
