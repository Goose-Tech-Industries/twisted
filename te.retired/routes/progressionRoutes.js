...
async function tryAutoLevelUps(characterId, userId, state) {
  ...
  for (let i = 0; i < 50; i += 1) {
    ...
    // Read level rewards from level_requirements table
    try {
      const [lvlRow] = await db.query('SELECT * FROM level_requirements WHERE level=?', [level]);
      if (lvlRow.length) {
        const lr = lvlRow[0];
        // AP reward (from table, falls back to setting)
        const ap = lr.ap_reward || 5;
        await db.query('UPDATE characters SET unspent_ap=unspent_ap+? WHERE id=?', [ap, characterId]);
        // Gold reward
        if (lr.gold_reward > 0) {
          await db.query('UPDATE users SET currency=currency+? WHERE id=?', [lr.gold_reward, userId]);
          if (!state.progression.level_rewards) state.progression.level_rewards = [];
          state.progression.level_rewards.push({ level, gold: lr.gold_reward });
        }
        // HP/MP bonus
        if (lr.hp_bonus > 0) await db.query('UPDATE characters SET max_hp=max_hp+?, current_hp=current_hp+? WHERE id=?', [lr.hp_bonus, lr.hp_bonus, characterId]);
        if (lr.mp_bonus > 0) await db.query('UPDATE characters SET max_mp=max_mp+?, current_mp=current_mp+? WHERE id=?', [lr.mp_bonus, lr.mp_bonus, characterId]);
        // Stat bonuses
        if (lr.stat_bonus_json) {
          const bonuses = typeof lr.stat_bonus_json === 'string' ? JSON.parse(lr.stat_bonus_json) : lr.stat_bonus_json;
          // Map legacy/admin names to actual DB columns
          const statMap = { strength: 'atk', defense: 'def', magic_offense: 'mo', magic_defense: 'md', atk: 'atk', def: 'def', mo: 'mo', md: 'md', speed: 'speed', luck: 'luck' };
          for (const [stat, val] of Object.entries(bonuses)) {
            const col = statMap[stat];
            if (col) {
              await db.query(`UPDATE characters SET \`${col}\`=\`${col}\`+? WHERE id=?`, [val, characterId]);
            }
          }
        }
        // Title unlock
        if (lr.unlock_title) {
          if (!state.progression.unlocked_titles) state.progression.unlocked_titles = [];
          state.progression.unlocked_titles.push(lr.unlock_title);
        }
      }
    } catch {}
  }
  ...
  if (levelsGained > 0) {
    ...
    // ── ACHIEVEMENT: level_reached trigger ───────────────────────
    try {
        const achievementRoutes = require('./routes/achievementRoutes');
        await achievementRoutes.checkForCharacter(db, null, characterId, 'level_reached', level);
...
router.post('/level-up', async (req, res) => {
  try {
    const userId = uid(req);
    const { characterId, spend } = req.body;
    const { character, state } = await loadCharacterOwned(userId, characterId);
