...
export type EntityType =
  ...
  | 'map_hazard'       // MapHazardPanel       → game_map_hazards
  | 'status_immunity'  // StatusImmunityPanel  → game_status_immunities
  | 'world'            // WorldPanel           → game_worlds
  | 'formation_shapes' // FormationShapesPanel → game_formation_shapes
  | 'sound_zone'       // SoundZonePanel       → game_map_sound_zones
  | 'tile_animation'   // TileAnimPanel        → game_tile_animations
  | 'npc_relationship' // RelationshipPanel   → game_npc_relationships
  | 'economy_state'    // EconomyPanel        → game_economy_state
  | 'world_event'      // WorldEventPanel     → game_world_events
  | 'cutscene'         // CutscenePanel       → game_cutscenes
  | 'economy_tx'       // EconomyTxPanel      → game_economy_transactions
  | 'inn'              // InnPanel            → game_inns
  | 'gathering_skill'  // GatheringSkillPanel → game_gathering_skills
  | 'gathering_node'   // GatheringNodePanel  → game_gathering_nodes
  | 'capture_item'     // CaptureItemPanel    → game_capture_items
  | 'creature'         // CreaturePanel       → character_creatures
  | 'season_effect'    // SeasonEffectPanel   → game_season_effects
  | 'bounty_board'     // BountyBoardPanel    → game_bounty_boards
  | 'bounty_task'      // BountyTaskPanel     → game_bounty_tasks
  | 'treasure_trail'   // TreasureTrailPanel  → game_treasure_trails
  | 'mount'            // MountPanel          → game_mounts
  | 'housing_plot'     // HousingPlotPanel    → game_housing_plots
  | 'furniture'        // FurniturePanel      → game_furniture
  | 'webhook'          // WebhookPanel        → game_webhooks
  | 'role_section'     // RoleSectionPanel    → admin_role_sections
  | 'terrain_interaction' // TerrainInteractionPanel → game_terrain_interactions
  | 'raid_boss'        // RaidBossPanel       → game_raid_bosses
  | 'job'              // JobPanel            → game_jobs
  | 'job_skill'        // JobSkillPanel       → game_job_skills
  | 'card'             // CardPanel           → game_cards
  | 'card_rule'        // CardRulePanel       → game_card_rules
  | 'siege_structure'  // SiegePanel          → game_siege_structures
  | 'dice_table'       // DicePanel           → game_dice_tables
  | 'fishing_spot'     // FishingPanel        → game_fishing_spots
  | 'puzzle'           // PuzzlePanel         → game_puzzles
  | 'minigame'         // MinigamePanel       → game_minigames
  | 'battle_referee'   // RefereePanel        → game_battle_referees
  | 'level_req'        // LevelTablePanel     → level_requirements
  | 'template'         // TemplatePanel       → game_templates
  | 'tile_type'        // TileTypePanel       → game_tile_types
  | 'theme'            // ThemePanel          → game_themes
  | 'capsule'          // CapsulePanel        → game_capsule_items
  | 'structure_template' // StructureTemplatePanel
  | 'ground_item'      // GroundItemPanel     → game_map_ground_items
  | 'deployed_structure' // DeployedStructurePanel
  | 'dm_campaign'       // DmCampaignPanel
  | 'dm_session_log'    // DmSessionLogPanel
  | 'scheduled_event'   // ScheduledEventPanel
  | 'autotile_group'   // AutotilePanel       → game_autotile_groups
  | 'companion_affinity_tier' // CompanionAffinityPanel
  | 'companion_quest'  // CompanionQuestPanel → companion_quests
  | 'referral'         // ReferralPanel       → referrals
