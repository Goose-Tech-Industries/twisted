...
import { GameplayHubPanel }      from "@/components/admin/gameplay-hub-panel"
import { SetupWizard }           from "@/components/admin/setup-wizard"
import { CommandSearch }         from "@/components/admin/command-search"
import { ShoutboxBar }           from "@/components/admin/shoutbox-bar"
...
function SectionContent({ section, onNavigate }: { section: AdminSection; onNavigate: (s: AdminSection) => void }) {
  switch (section) {
    ...
    case 'gameplay':          return <GameplayHubPanel />
...
export default function AdminSaucePage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/admin-panel/setup-status`, { credentials: 'include' })
      .then(r => r.json())
      .then(d => { if (d.success && !d.completed) setShowWizard(true) })
      .catch(() => {})
  }, [])
  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto min-w-0 flex flex-col pt-14 md:pt-0 pb-10">
        <GmNotesStrip />
      ...
      <CommandSearch onNavigate={handleSectionChange} />
