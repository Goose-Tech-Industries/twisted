...
export function PartyPanel() {
  const { state, socket, partyAccept, partyDecline, partyKick } = useGame()
  ...
  const [pendingInvite, setPendingInvite] = useState<{ partyId: number; inviterName: string } | null>(null)
  ...
  useEffect(() => {
    if (!socket) return
    const onInvited = (data: { partyId: number; inviterName: string }) => {
      setPendingInvite(data)
    }
    socket.on('party_invited', onInvited)
    return () => { socket.off('party_invited', onInvited) }
  }, [socket])
  ...
  return (
    <div className="flex h-full max-w-5xl mx-auto w-full">
      {/* Main Content */}
        ...
        <div className="max-w-3xl mx-auto">
          ...
          {pendingInvite && (
            <div className="mb-4 p-3 rounded-lg border border-primary/30 bg-primary/5 flex items-center justify-between">
              <span className="text-sm"><strong>{pendingInvite.inviterName}</strong> invited you to a party</span>
              <div className="flex gap-1">
                <Button size="sm" variant="default" onClick={() => { partyAccept(pendingInvite.partyId); setPendingInvite(null); setTimeout(fetchParty, 500) }}>Accept</Button>
                <Button size="sm" variant="outline" onClick={() => { partyDecline(pendingInvite.partyId); setPendingInvite(null) }}>Decline</Button>
              </div>
            </div>
          )}
          ...
          <div className="space-y-2">
            ...
            {selectedMember.char_id !== charId && selectedMember.char_id !== party?.leader_id && (
              <Button className="w-full border-destructive/50 text-destructive hover:bg-destructive/10" variant="outline"
                onClick={() => { partyKick(selectedMember.char_id); setSelectedMember(null); fetchParty() }}>
