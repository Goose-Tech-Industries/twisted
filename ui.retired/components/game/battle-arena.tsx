...
import { getParticleForAction } from "./particle-overlay"
...
import { CombatantCard, MiniCombatantPill } from "./combatant-card"
import { BattleGrid } from "./battle-grid"

// ── Shared constants (exported for sub-components) ──────────────────────

export const COMMAND_ICONS: Record<string, React.ElementType> = {
  attack: Swords, skill: Sparkles, item: FlaskRound,
  defend: Shield, flee: LogOut, limit: Zap
}

export const TERRAIN_STYLES: Record<string, { bg: string; label: string; icon: string; difficult?: boolean }> = {
  ...
  water:       { bg: '#0a1a3a', label: 'Water',        icon: '🌊', difficult: true },
  fire:        { bg: '#3a1a0a', label: 'Fire! 8 dmg', icon: '🔥' },
  mud:         { bg: '#2a1a0a', label: 'Mud (slow)',   icon: '🟤', difficult: true },
  ice:         { bg: '#1a2a3a', label: 'Ice (slow)',   icon: '🧊', difficult: true },
  rubble:      { bg: '#2a2a2a', label: 'Rubble (slow)', icon: '🪨', difficult: true },
  sand:        { bg: '#3a3a1a', label: 'Sand (slow)',  icon: '🏜️', difficult: true },
  swamp:       { bg: '#1a2a1a', label: 'Swamp (slow)', icon: '🌿', difficult: true },
  thorns:      { bg: '#1a2a0a', label: 'Thorns (slow+dmg)', icon: '🌹', difficult: true },
}

// Elevation colors — higher = lighter
export const ELEVATION_OPACITY = [0, 0.08, 0.16, 0.24]

const CELL_SIZE_DESKTOP = 64
const CELL_SIZE_MOBILE = 40

export const WOUND_COLORS: Record<string, string> = {
...
export function BattleArena() {
  const { state, dispatch, socket, battleBrave, battleDefault, battleAutoToggle, battlePreview, battleChatSend } = useGame()
  const { notify } = useNotification()
  const haptics = useHaptics()

  const battle = state.battle
  const isSpectator = !!(battle as Record<string, unknown> | null)?.isSpectator
  const [isMobileBattle, setIsMobileBattle] = useState(false)
  useEffect(() => {
    const mql = window.matchMedia('(max-width: 767px)')
    const onChange = () => setIsMobileBattle(mql.matches)
    mql.addEventListener('change', onChange)
    setIsMobileBattle(mql.matches)
    return () => mql.removeEventListener('change', onChange)
  }, [])
  const CELL_SIZE = isMobileBattle ? CELL_SIZE_MOBILE : CELL_SIZE_DESKTOP
  ...
  useEffect(() => {
    ...
    const handler = (data: { state: unknown; action: { actions?: Array<{ type: string; target?: string; elements?: string[] }> } | null }) => {
      ...
      for (const a of data.action.actions) {
        // Check if we're the target (damage taken)
        if (a.target && battle?.me?.name === a.target && (a.type === 'attack' || a.type === 'skill')) tookDamage = true
        const elements = (a as unknown as Record<string, unknown>).elements as string[] | undefined
  ...
  const elevationMap = (grid as unknown as Record<string, unknown>).elevation as Record<string, number> || {}
  const threatenedTiles = new Set(((grid as unknown as Record<string, unknown>).threatenedTiles as string[]) || [])
  const difficultTerrainTiles = ((grid as unknown as Record<string, unknown>).difficultTerrain as Array<{ x: number; y: number; terrain: string }>) || []
  const difficultSet = new Set(difficultTerrainTiles.map(d => `${d.x},${d.y}`))
  ...
  return (
    <div className="relative flex flex-col h-full md:h-full">
    <div className="flex flex-col h-full md:h-full">
      {/* Spectator Banner */}
      {isSpectator && (
        <div className="bg-[oklch(0.30_0.10_250)] border-b border-[oklch(0.40_0.10_250)] px-3 py-2 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2 text-sm text-[oklch(0.80_0.10_250)]">
      ...
      {/* Top Bar: Turn Order */}
      <div className="border-b border-border bg-card/80 px-2 md:px-4 py-1.5 md:py-2 flex items-center gap-2 md:gap-4 shrink-0">
        <div className="flex items-center gap-1 text-xs text-muted-foreground shrink-0">
          <Clock className="w-3.5 h-3.5" /> <span className="hidden md:inline">Turn</span> {battle.turn}
        </div>
        {battle.weather && battle.weather.name !== 'clear' && (
          <div className="hidden md:flex items-center gap-1 px-2 py-0.5 rounded bg-muted/30 text-[10px] text-muted-foreground">
            <span>{battle.weather.icon}</span>
            <span>{battle.weather.label}</span>
          </div>
        )}
        {battle.winCondition && (
          <div className="hidden md:flex items-center gap-1 px-2 py-0.5 rounded bg-[oklch(0.55_0.15_140)]/10 text-[10px] text-[oklch(0.55_0.15_140)]">
            <span>{battle.winCondition.icon}</span>
            <span>{battle.winCondition.description}</span>
          </div>
        )}
        <div className="flex items-center gap-1 flex-1 overflow-x-auto scrollbar-none snap-x">
          {turnOrder.map((entry, i) => (
            <div
              key={`${entry.charId}-${i}`}
              className={cn(
                "flex items-center gap-1 px-1.5 md:px-2 py-0.5 md:py-1 rounded text-[10px] md:text-xs whitespace-nowrap snap-start shrink-0",
        ...
        <div className={cn(
          "px-2 md:px-3 py-0.5 md:py-1 rounded-full text-[10px] md:text-xs font-medium shrink-0",
      ...
      {/* Mobile: Horizontal team strips */}
      <div className="md:hidden border-b border-border bg-card/50 overflow-x-auto shrink-0">
        <div className="flex gap-1.5 p-1.5">
          {playerTeam.filter(c => c).map((c, i) => (
            <MiniCombatantPill key={c.charId || i} combatant={c} isActive={c.charId === battle.turnCharId} color="primary" />
          ))}
          <div className="w-px bg-border shrink-0 my-1" />
          {enemyTeam.filter(c => c).map((c, i) => (
            <MiniCombatantPill
              key={c.charId || i} combatant={c} isActive={c.charId === battle.turnCharId} color="destructive"
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Left: Your Team — desktop only */}
        <div className="hidden md:block w-48 border-r border-border bg-card/50 p-2 overflow-y-auto space-y-2">
        ...
        <BattleGrid
          ...
          myCharId={myCharId}
          myTeamId={myTeamId}
          turnCharId={battle.turnCharId}
          CELL_SIZE={CELL_SIZE}
          isAnimating={isAnimating}
          particleEffects={particleEffects}
          battleEmote={state.battleEmote}
          battle={battle as unknown as Record<string, unknown>}
          ctb={(battle as unknown as Record<string, unknown>).ctb as Record<string, unknown> | null}
          onGridClick={handleGridClick}
          isMyTurn={!!battle.isMyTurn}
          hasMoved={!!battle.hasMoved}
          showMoveRange={showMoveRange}
          moveRange={battle.moveRange || 2}
          onToggleMove={() => setShowMoveRange(!showMoveRange)}
          hasFormation={!!(battle as unknown as Record<string, unknown>).formation}
          onSwapRow={() => socket?.emit('battle_swap_row', { battleId: battle.battleId })}
        />

        {/* Right: Enemy Team(s) — desktop only */}
        <div className="hidden md:block w-48 border-l border-border bg-card/50 p-2 overflow-y-auto space-y-2">
      ...
      <div className="border-t border-border bg-card shrink-0">
        {/* Battle Log (left) + Chat (right) — chat hidden on mobile */}
        <div className="flex border-b border-border/50">
          {/* Battle Log */}
          <div className="flex-1 px-2 md:px-3 py-1.5 md:py-2 max-h-16 md:max-h-24 overflow-y-auto md:border-r border-border/30">
          ...
          {/* Battle Chat — desktop only */}
          <div className="hidden md:flex w-64 px-3 py-2 max-h-24 flex-col">
              ...
              {selectedCommand.skills.map(skill => {
                ...
                return (
                  <button
                    ...
                    onMouseEnter={() => { if (canUse && selectedTarget != null) battlePreview(skill.id, selectedTarget) }}
        ...
        {/* Main Commands */}
        {!selectedCommand && (
          <div className="flex flex-wrap gap-1.5 md:gap-2 justify-center px-2 md:px-4 py-2 md:py-3">
            ...
            {/* Auto-battle toggle */}
            {(() => {
              const autoOn = !!(battle as unknown as Record<string, unknown>).autoEnabled
              return (
                <Button
                  variant={autoOn ? "default" : "outline"}
                  size="sm"
                  className={autoOn ? "bg-amber-700 hover:bg-amber-800" : ""}
                  onClick={() => battleAutoToggle(!autoOn)}
                >
