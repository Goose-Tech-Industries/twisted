...
export function ChatPanel({ embedded }: { embedded?: boolean } = {}) {
  const { socket, character, isStaff, emote, typingDM } = useGame()
  ...
  // Emit typingDM when typing in DM channel (debounced)
  const handleInputChange = useCallback((value: string) => {
    setInputValue(value)
    if (activeChannel === 'dm' && dmTarget?.charId) {
      if (typingTimerRef.current) clearTimeout(typingTimerRef.current)
      typingTimerRef.current = setTimeout(() => {
        typingDM(dmTarget.charId!)
      }, 300)
    }
  }, [activeChannel, dmTarget, typingDM])
  ...
  const sendMessage = useCallback(() => {
    ...
    const emoteMatch = inputValue.trim().match(/^\/(?:emote|e)\s+(.+)/i)
    if (emoteMatch) {
      emote(emoteMatch[1])
      setInputValue('')
      return
    ...
    const payload: { channel: string; text: string; targetCharId?: number | null } = {
  ...
  const visibleMessages: ChatMessage[] = messages[activeChannel] || []
  const systemMessages: ChatMessage[] = messages.system || []

  // Get channel config
  const channelConfig = CHANNELS.find(c => c.key === activeChannel)

  // Filter channels for non-staff users
  const availableChannels = CHANNELS.filter(c => !('staffOnly' in c && c.staffOnly) || isStaff)

  // ── Embedded mode (for mobile chat sheet) ──
  if (embedded) {
    return (
      <div className="flex flex-col h-full bg-card rounded-lg">
        <div className="flex gap-1 p-2 overflow-x-auto scrollbar-none shrink-0 snap-x">
          {availableChannels.map(channel => {
            const Icon = channel.icon
            const isActive = activeChannel === channel.key
            const unreadCount = unread[channel.key] || 0
            return (
              <button key={channel.key} onClick={() => switchChannel(channel.key)}
                className={cn("relative flex items-center gap-1.5 px-2.5 py-1.5 rounded text-xs whitespace-nowrap transition-all",
                  isActive ? "bg-secondary/80 text-foreground border border-border" : "text-muted-foreground hover:text-foreground hover:bg-secondary/30")}>
        ...
        {activeChannel === 'dm' && (
          <div className="flex items-center gap-2 px-3 py-2 bg-pink-500/5 border-b border-pink-500/20 shrink-0">
            <input ref={dmInputRef} type="text" value={dmInputValue}
              onChange={(e) => setDmInputValue(e.target.value)}
              onKeyDown={(e) => { e.stopPropagation(); if (e.key === 'Enter') setDmTargetFromName(dmInputValue) }}
            ...
            {dmTarget && <span className="text-xs text-pink-400">{'→'} {dmTarget.name}</span>}
          </div>
        )}
        <div className="flex-1 overflow-y-auto p-2 flex flex-col gap-1 min-h-0">
          {systemMessages.map(msg => (
            <div key={msg.id}><ChatLine msg={msg as ChatMessage} myCharId={character?.charId} onDM={startDM} /></div>
          ))}
        ...
        <div className="flex gap-2 p-2 border-t border-border shrink-0">
          <input ref={inputRef} type="text" value={inputValue}
            ...
            onKeyDown={(e) => { e.stopPropagation(); if (e.key === 'Enter') sendMessage(); if (e.key === 'Escape') inputRef.current?.blur() }}
            placeholder={`${channelConfig?.label || 'Message'}...`} maxLength={300}
            className="flex-1 bg-background/50 border border-border px-3 py-2 text-sm rounded outline-none focus:border-primary" />
    ...
    return (
      <div className="fixed z-40 bottom-4 left-4">
        ...
        >
          ...
          {(Object.values(unread) as number[]).reduce((a: number, b: number) => a + b, 0) > 0 && (
            <span className="bg-destructive text-destructive-foreground text-xs px-1.5 rounded-full">
              {(Object.values(unread) as number[]).reduce((a: number, b: number) => a + b, 0)}
            </span>
          )}
        </button>
      </div>
  ...
  return (
    <div className="fixed z-40 bottom-4 left-4 w-[360px] bg-card/95 backdrop-blur border border-border rounded-lg shadow-xl">
      {/* Header */}
      <div 
      ...
      {/* Channel Tabs */}
      <div className="flex gap-1 p-2 overflow-x-auto scrollbar-none snap-x">
        {availableChannels.map(channel => {
          const Icon = channel.icon
          const isActive = activeChannel === channel.key
          const unreadCount = unread[channel.key] || 0
      ...
      {/* Messages */}
      <div className="h-32 md:h-48 overflow-y-auto p-2 flex flex-col gap-1 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-border">
      ...
      <div className="flex gap-2 p-2 border-t border-border">
        <input
          ...
          style={{
            borderColor: activeChannel !== 'global' && channelConfig ? undefined : undefined
          }}
        />
        <button
          onClick={sendMessage}
