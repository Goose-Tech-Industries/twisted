...
import { GuildBankPanel } from './guild-bank-panel'

export function GuildPanel() {
  const { state, socket, notify, guildSetRank } = useGame()
  ...
  const createGuild = useCallback(async () => {
    ...
    if (res.success) {
      notify('success', `Guild "${res.data?.guildName}" created!`)
      setIsCreating(false)
      setCreateForm({ name: '', tag: '', description: '', emblem: '' })
      loadGuildData()
    } else {
      notify('error', res.error || 'Failed to create guild')
    }
  }, [charId, createForm, notify, loadGuildData])
  
  const acceptInvite = useCallback(async (guildId: number) => {
    if (!socket) return
    socket.emit('guild_accept', { guildId })
    notify('success', 'Joined guild!')
    setTimeout(loadGuildData, 500)
  }, [socket, notify, loadGuildData])
  ...
  const disbandGuild = useCallback(async () => {
    ...
    if (res.success) {
      notify('warning', 'Guild disbanded')
      loadGuildData()
    }
  }, [charId, myGuild, notify, loadGuildData])
  ...
  const promoteMember = useCallback((targetCharId: number) => {
    guildSetRank(targetCharId, 'OFFICER')
    setTimeout(loadGuildData, 500)
  }, [guildSetRank, loadGuildData])

  const demoteMember = useCallback((targetCharId: number) => {
    guildSetRank(targetCharId, 'MEMBER')
    setTimeout(loadGuildData, 500)
  }, [guildSetRank, loadGuildData])
  ...
  if (isLoading) {
    return (
      <Card className="celtic-border h-full max-w-5xl mx-auto w-full">
      ...
      <CardContent className="p-0">
        <Tabs defaultValue="members">
          <TabsList className="w-full rounded-none border-b border-border bg-transparent">
            ...
            <TabsTrigger value="bank" className="flex-1 data-[state=active]:bg-secondary/50">
          ...
          <TabsContent value="bank" className="mt-0">
            <GuildBankPanel guildId={myGuild!.id} guildName={myGuild!.name} myRank={myGuild!.rank} />
          </TabsContent>
