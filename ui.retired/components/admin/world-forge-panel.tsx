...
type GenerateMode = 'town' | 'npcs' | 'quest_chain' | 'full_world' | 'dungeon'
...
export function WorldForgePanel() {
  ...
  const [dungeonTheme, setDungeonTheme] = useState('An ancient barrow beneath the hill where druids once sealed something terrible')
  const [dungeonSize, setDungeonSize] = useState('medium')
  ...
  const generate = async () => {
    ...
    switch (mode) {
      ...
      case 'dungeon':     params = { theme: dungeonTheme, size: dungeonSize }; break
    }
    try {
  ...
  const commit = async () => {
    if (!result) { alert('No data to commit.'); return }
    setView('generating')
    try {
      ...
      if (!r.ok) { setErrorMsg(`Server error: ${r.status} ${r.statusText}`); setView('error'); return }
    ...
    } catch (e: unknown) { setErrorMsg('Commit failed: ' + String(e)); setView('error') }
  }

  const modeLabel = (m: GenerateMode): string =>
    ({ town: 'town', npcs: 'NPCs', quest_chain: 'quest chain', full_world: 'full world', dungeon: 'dungeon' })[m]
        ...
        <div className="mb-6">
          ...
          <div className="p-4 bg-card border border-blue-900/40 rounded-lg">
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              {(result.map as Record<string,unknown>).layout_type ? <Pill>🏗️ {String((result.map as Record<string,unknown>).layout_type)} layout</Pill> : null}
              {(result.map as Record<string,unknown>).fog_of_war ? <Pill>🌫️ fog of war</Pill> : null}
            </div>
          </div>
        </div>
      )}
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div className={cardBase}>
          <div className="text-xs font-bold uppercase tracking-widest text-red-400 mb-3">🏰 Generate Dungeon</div>
          <p className="text-xs text-muted-foreground mb-3 leading-relaxed">
            Generates a dungeon with <b>procedural tile layout</b> (BSP rooms + corridors), enemies, a boss, loot, and quests. Fog of war enabled by default.
          </p>
          <Label>Theme</Label>
          <Input value={dungeonTheme} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setDungeonTheme(e.target.value)} placeholder="e.g. flooded crypt, spider nest, abandoned mine" />
          <Label>Size</Label>
          <Sel value={dungeonSize} onChange={setDungeonSize}>
            <option value="small">Small (20×16) — 3-4 rooms</option>
            <option value="medium">Medium (30×24) — 5-7 rooms</option>
            <option value="large">Large (40×32) — 8-10 rooms</option>
          </Sel>
          <button onClick={() => { setMode('dungeon'); generate() }}
            className={cn(btnBase, 'bg-gradient-to-r from-red-700 to-red-900 text-white')}>
            🏰 Forge Dungeon
          </button>
        </div>
