...
import { useQuickSlotKeys } from '@/hooks/use-keyboard'
...
export function QuickSlots() {
  ...
  useQuickSlotKeys(handleUseSlot)
  ...
  return (
    <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-40">
      <div className="flex gap-1 p-2 bg-card/90 backdrop-blur-sm border border-border rounded-lg shadow-lg">
        {slots.map((slot, index) => {
          ...
          return (
            <div
              ...
              onDrop={(e) => {
                e.preventDefault()
                // Check for item data from inventory drag
            ...
            >
              <button
                ...
                `}
              >
                <Icon className={`w-6 h-6 ${slot.item ? 'text-foreground' : 'text-muted-foreground'}`} />
                
                {/* Stack count */}
                {slot.item && slot.item.quantity && slot.item.quantity > 1 && (
                  <span className="absolute bottom-0.5 right-0.5 text-[10px] font-bold text-foreground bg-black/60 px-1 rounded">
                    {slot.item.quantity}
                  </span>
                )}
              </button>
              ...
              {/* Keybind indicator */}
              <div className="absolute -bottom-5 left-1/2 -translate-x-1/2 text-[10px] text-muted-foreground font-mono whitespace-nowrap">
        ...
        {/* Auto-fill from consumables hint */}
