...
export function TradePanel() {
  ...
  useEffect(() => {
    ...
    const onTradeComplete = () => {
      ...
      setTradeId(null)
      setTrade(null)
      actions.notify('success', 'Trade complete!')
      // Refresh inventory
      if (state.character) {
        actions.loadCharacter(state.character.charId)
      }
    }
    ...
    const onTradeCancelled = (data: { reason?: string }) => {
      ...
      setTradeId(null)
      setTrade(null)
      actions.notify('warning', data.reason || 'Trade cancelled')
    }
  ...
  }, [socket, tradeId, actions, state.character])
