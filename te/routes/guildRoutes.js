...
router.post('/bank/deposit-gold', async (req, res) => {
    ...
    try {
        ...
        if ((userRow?.currency || 0) < gold) return res.json({ success: false, error: 'Not enough gold.' });

        await db.query('UPDATE users SET currency=currency-? WHERE id=?', [gold, userId]);
        await db.query('UPDATE guilds SET gold_bank=gold_bank+? WHERE id=?', [gold, guildId]);
...
router.post('/bank/withdraw-gold', async (req, res) => {
    ...
    try {
        ...
        const [[guild]] = await db.query('SELECT gold_bank FROM guilds WHERE id=?', [guildId]);
        if ((guild?.gold_bank || 0) < gold) return res.json({ success: false, error: 'Bank has insufficient gold.' });
