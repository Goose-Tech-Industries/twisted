...
const Panels = {
    ...
    _charSheetHTML(d, activeTab) {
        ...
        switch (activeTab) {
            ...
            case 'grimoire':    content = Panels._csGrimoire();             break;
            case 'pvp':         content = Panels._csPvpRoster();        break;
            case 'crafting':    content = Panels._csCrafting();         break;
        }
    ...
    async buyItem(itemId, price, name) {
        ...
        if (j.success) { delete Panels.shopDiscount; await Panels.openShop(Panels.shopId); } // Refresh
    },
    ...
    // =============================================================
    // ── PvP ROSTER TAB ────────────────────────────────────────────
    // TEACHING: This lets a player build their 3v3 battle team from
    // their owned characters. We store charIds in character_pvp_teams
    // (no localStorage). The tab fetches on open and saves on button click.
    // ── CRAFTING TAB ──────────────────────────────────────────────
    _craftData: null,
    ...
    async _craftDo(recipeId) {
        try {
            ...
            if (j.success) {
                Panels._craftData = null;  // invalidate cache so it reloads
                Panels._charTabSwitch('crafting');
            }
    ...
    _csCrafting() {
        ...
        html += `<div style="margin-top:12px">
            <button onclick="Panels._craftData=null;Panels._charTabSwitch('crafting')"
    ...
    async _csPvpRosterInit() {
        try {
            ...
            const j = await r.json();
            if (j.success) { Panels._pvpRosterData = j; Panels._charTabSwitch('pvp'); }
    ...
    async _savePvpRoster() {
        ...
        const maxTeam = Panels._pvpRosterData?.maxTeam || 3;
        ...
        try {
            ...
            const j = await r.json();
            if (j.success) {
                Panels._pvpRosterData.teamCharIds = j.teamCharIds;
    ...
    _csPvpRoster() {
        const d = Panels._pvpRosterData;

        // First load: show a loading state and kick off the fetch
        if (!d) {
            // Async — will re-render once loaded
            Panels._csPvpRosterInit();
        ...
        <div style="max-width:480px;margin:0 auto">
            ...
            <div style="display:flex;gap:10px;align-items:center">
                ...
                <button onclick="Panels._pvpRosterData=null;Panels._charTabSwitch('pvp')"
    ...
    // ---- GRIMOIRE TAB ----
    // Shows all slotted Blood Oghams, their rank progress, lore, and active set bonuses.
    // Data is loaded async and injected into a placeholder div.
    _csGrimoire() {
