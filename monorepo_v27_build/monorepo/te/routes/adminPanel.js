...
const ENTITY_PK_MAP = {
    artifact: 'artifact_id', quest: 'quest_id',
    loot_table: 'id', spawn: 'id'
};
...
router.get('/:type(item|skill|npc|map|quest|class|race|ogham|ogham_family|shop|arena|artifact|status|feat|loot_table|spawn|battle_cmd|background)', requireStaff, async (req, res) => {
    try {
...
router.post('/:type(item|skill|npc|map|quest|class|race|ogham|ogham_family|shop|arena|artifact|status|feat|loot_table|spawn|battle_cmd|background)', requireStaff, async (req, res) => {
    try {
        const table = getTable(req.params.type);
        const pk = getPk(req.params.type);
        const data = req.body;
