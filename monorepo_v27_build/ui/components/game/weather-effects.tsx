...
export function WeatherEffects({ weather = 'clear' }: { weather?: WeatherType }) {
  ...
  // Animation loop
  useEffect(() => {
    if (!canvasRef.current || weather === 'clear' || settings.reducedMotion) return
  ...
  }, [weather, dimensions, settings.reducedMotion])
  
  if (weather === 'clear' || settings.reducedMotion) return null
