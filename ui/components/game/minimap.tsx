...
// Tile color palette matching the original MinimapUI
const TILE_COLORS = [
...
export function Minimap({ size = 160, tileSize = 4 }: MinimapProps) {
  const { state } = useGame()
  ...
  return (
    <div className="fixed bottom-4 right-4 z-30">
      <div className="text-[9px] text-muted-foreground/40 text-right mb-1 uppercase tracking-wider">
        Minimap
      </div>
      <canvas
        ref={canvasRef}
        width={size}
        height={size}
        className="rounded-md border border-border/40 bg-card/90 backdrop-blur cursor-default"
        style={{ 
