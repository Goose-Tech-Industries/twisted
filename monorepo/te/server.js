...
const sessionMiddleware = session({
    ...
    cookie: {
        httpOnly: true,
        // TEACHING: In production, UI and API share one domain via nginx → sameSite:'lax' is correct.
        // In dev, UI(:3000) and API(:3001) are different origins → we need sameSite:'none' so the
        // browser will send the cookie cross-origin. sameSite:'none' requires secure:true per spec,
        // but secure cookies only transmit over HTTPS. For local HTTP dev this pair won't work in
        // strict browsers — the recommended dev workaround is to run both on the same port via a
        // local proxy (see README) OR just use Chrome with --disable-web-security for local testing.
    ...
    try {
        ...
        app.post('/admin/clear-cache', async (req, res) => {
            try {
                ...
                const { mapId, reloadRegions } = req.body;
        ...
        io.on('connection', (socket) => {
            ...
            socket.on('select_character', async (data) => {
                // Delegate to join_game by sharing the same handler.
                // Re-extract session userId and run full join flow.
                try {
                    const sessionData = socket.request.session;
                    const userId = sessionData && sessionData.userId;
                    ...
                    // Fire join_game internally by delegating to the socket
                    socket.emit('_internal_join', { charId });
                } catch (err) { console.error('select_character error:', err); }
            });
            // _internal_join — used by select_character alias
            socket.on('_internal_join', (data) => {
                // Trigger join_game logic with same data
                socket.listeners('join_game')[0]?.(data);
            });
